...
var layer = {
  ...
  use: function(module, fn, readyMethod){
    ...
    var id = 'layui_layer_' + module.replace(starc.cache.extend_dir, '').replace(/\.|\//g, '');
  ...
  ready: function(fn){
    starc.addcss('pc/layer/layer.css', fn, 'skinlayercss');
    return this;
  },
...
var Class = function(setings){
  var that = this;
  //lijw-id排重--当前只针对layui-layer--begin
  (function loop(n) {
		if($("#layui-layer"+(layer.index+1))[0]) {
			layer.index++;
			loop(n+1);
		} else {
			return;
  ...
  callback([
    ...
    '<div class="'+ doms[0] + (' layui-layer-'+ready.type[config.type]) + (((config.type == 0 || config.type == 2) && !config.shade) ? ' layui-layer-border' : '') + ' ' + (config.skin||'') +'" id="'+ doms[0] + times +'" type="'+ ready.type[config.type] +'" times="'+ times +'" showtime="'+ config.time +'" conType="'+ (conType ? 'object' : 'string') +'" style="z-index: '+ zIndex +'; width:'+ config.area[0] + ';height:' + config.area[1] + (config.fix ? '' : ';position:absolute;') +'">'
      ...
      + '<span class="layui-layer-setwin">'+ function(){
        var closebtn = ismax ? '<a target="_self" class="layui-layer-min" href="javascript:void(0);"><cite></cite></a><a class="layui-layer-ico layui-layer-max" href="javascript:void(0);"></a>' : '';
        config.closeBtn && (closebtn += '<a  target="_self" class="layui-layer-ico '+ doms[7] +' '+ doms[7] + (config.title ? config.closeBtn : (config.type == 4 ? '1' : '2')) +'" href="javascript:void(0);"></a>');
        return closebtn;
      }() + '</span>'
      + (config.btn ? function(){
        var button = '';
    ...
    dict.index = layer.open($.extend({
      ...
      content: '<div class="layui-layer-phimg">'
        ...
        +'<div class="layui-layer-imgsee">'
          +(data.length > 1 ? '<span class="layui-layer-imguide"><a href="javascript:void(0);" class="layui-layer-iconext layui-layer-imgprev"></a><a href="javascript:void(0);" class="layui-layer-iconext layui-layer-imgnext"></a></span>' : '')
          +'<div class="layui-layer-imgbar" style="display:'+ (key ? 'block' : '') +'"><span class="layui-layer-imgtit"><a href="javascript:void(0);">'+ (data[start].alt||'') +'</a><em>'+ dict.imgIndex +'/'+ data.length +'</em></span></div>'
        +'</div>'
      +'</div>',
...
//加载组件所需的css
starc.addcss('modules/layer/layer.css', 'skinlayercss');

//定义模块
starc.define('jquery', function(exports){
  layer.path = starc.cache.dir;
  ready.run();
